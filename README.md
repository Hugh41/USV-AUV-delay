...
All plotting scripts live in `plot_figures/` and read from `delay_comparison_results/` produced by `compare_delay_stackelberg.py`.
...
To regenerate the full GIF set (2 / 3 / 4 AUVs):
...
# team_size_summary once (included in the n_auv=2 run above)
```

You can also run each script individually with more control:
...
These scripts use the fixed experiment directories hardcoded at the top of each file (`RESULT_DIR`). Run them from the repository root:
